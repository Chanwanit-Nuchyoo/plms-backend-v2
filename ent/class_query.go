...
type ClassQuery struct {
	...
	withMembers          *ClassUserRoleQuery
	withPosts            *PostQuery
	withClassLabStatuses *ClassLabStatusQuery
...
func (cq *ClassQuery) QueryOwner() *UserQuery {
...
func (cq *ClassQuery) QueryMembers() *ClassUserRoleQuery {
	query := (&ClassUserRoleClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *ClassQuery) QueryPosts() *PostQuery {
...
// QueryClassLabStatuses chains the current query on the "class_lab_statuses" edge.
func (cq *ClassQuery) QueryClassLabStatuses() *ClassLabStatusQuery {
...
func (cq *ClassQuery) Clone() *ClassQuery {
	...
	return &ClassQuery{
		...
		withClassLabStatuses: cq.withClassLabStatuses.Clone(),
...
func (cq *ClassQuery) WithOwner(opts ...func(*UserQuery)) *ClassQuery {
	query := (&UserClient{config: cq.config}).Query()
...
func (cq *ClassQuery) WithMembers(opts ...func(*ClassUserRoleQuery)) *ClassQuery {
	query := (&ClassUserRoleClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ClassQuery) WithPosts(opts ...func(*PostQuery)) *ClassQuery {
	query := (&PostClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithClassLabStatuses tells the query-builder to eager-load the nodes that are connected to
// the "class_lab_statuses" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *ClassQuery) WithClassLabStatuses(opts ...func(*ClassLabStatusQuery)) *ClassQuery {
	query := (&ClassLabStatusClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withClassLabStatuses = query
	return cq
}
...
func (cq *ClassQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Class, error) {
	...
	if query := cq.withOwner; query != nil {
		if err := cq.loadOwner(ctx, query, nodes, nil,
			func(n *Class, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := cq.withMembers; query != nil {
		if err := cq.loadMembers(ctx, query, nodes,
			func(n *Class) { n.Edges.Members = []*Class_User_Role{} },
			func(n *Class, e *Class_User_Role) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withPosts; query != nil {
		if err := cq.loadPosts(ctx, query, nodes,
			func(n *Class) { n.Edges.Posts = []*Post{} },
			func(n *Class, e *Post) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withClassLabStatuses; query != nil {
		if err := cq.loadClassLabStatuses(ctx, query, nodes,
			func(n *Class) { n.Edges.ClassLabStatuses = []*Class_Lab_Status{} },
			func(n *Class, e *Class_Lab_Status) { n.Edges.ClassLabStatuses = append(n.Edges.ClassLabStatuses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ClassQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Class, init func(*Class), assign func(*Class, *User)) error {
...
func (cq *ClassQuery) loadMembers(ctx context.Context, query *ClassUserRoleQuery, nodes []*Class, init func(*Class), assign func(*Class, *Class_User_Role)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(class_user_role.FieldClassID)
	}
	query.Where(predicate.Class_User_Role(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(class.MembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *ClassQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Class, init func(*Class), assign func(*Class, *Post)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Class)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(post.FieldClassID)
	}
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(class.PostsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *ClassQuery) loadClassLabStatuses(ctx context.Context, query *ClassLabStatusQuery, nodes []*Class, init func(*Class), assign func(*Class, *Class_Lab_Status)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Class)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(class_lab_status.FieldClassID)
	}
	query.Where(predicate.Class_Lab_Status(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(class.ClassLabStatusesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *ClassQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withOwner != nil {
			_spec.Node.AddColumnOnce(class.FieldOwnerID)
		}
		if cq.withCourse != nil {
			_spec.Node.AddColumnOnce(class.FieldCourseID)
		}
	}
