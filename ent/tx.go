...
func (tx *Tx) init() {
	...
	tx.Class_Lab_Status = NewClassLabStatusClient(tx.config)
	tx.Class_User_Role = NewClassUserRoleClient(tx.config)
	tx.Course = NewCourseClient(tx.config)
	tx.Lab = NewLabClient(tx.config)
	tx.Lab_Problem = NewLabProblemClient(tx.config)
	tx.Lab_Problem_Submission = NewLabProblemSubmissionClient(tx.config)
	tx.Post = NewPostClient(tx.config)
	tx.Testcase = NewTestcaseClient(tx.config)
	tx.Testcase_Submission = NewTestcaseSubmissionClient(tx.config)
	tx.Topic = NewTopicClient(tx.config)
	tx.Topic_File_Mats = NewTopicFileMatsClient(tx.config)
