...
type TopicEdges struct {
	...
	Files []*Topic_File_Mats `json:"files,omitempty"`
...
func (e TopicEdges) CourseOrErr() (*Course, error) {
	if e.loadedTypes[0] {
		if e.Course == nil {
...
func (e TopicEdges) FilesOrErr() ([]*Topic_File_Mats, error) {
	if e.loadedTypes[1] {
		return e.Files, nil
	}
...
func (*Topic) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case topic.FieldID, topic.FieldCourseID:
...
// QueryCourse queries the "course" edge of the Topic entity.
func (t *Topic) QueryCourse() *CourseQuery {
	return NewTopicClient(t.config).QueryCourse(t)
}

// QueryFiles queries the "files" edge of the Topic entity.
func (t *Topic) QueryFiles() *TopicFileMatsQuery {
	return NewTopicClient(t.config).QueryFiles(t)
}
...
func (t *Topic) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", t.CourseID))
	builder.WriteString(", ")
