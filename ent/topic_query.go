...
func (tq *TopicQuery) QueryCourse() *CourseQuery {
...
func (tq *TopicQuery) QueryFiles() *TopicFileMatsQuery {
	query := (&TopicFileMatsClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TopicQuery) WithCourse(opts ...func(*CourseQuery)) *TopicQuery {
	query := (&CourseClient{config: tq.config}).Query()
...
func (tq *TopicQuery) WithFiles(opts ...func(*TopicFileMatsQuery)) *TopicQuery {
	query := (&TopicFileMatsClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if query := tq.withCourse; query != nil {
		if err := tq.loadCourse(ctx, query, nodes, nil,
			func(n *Topic, e *Course) { n.Edges.Course = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withFiles; query != nil {
		if err := tq.loadFiles(ctx, query, nodes,
			func(n *Topic) { n.Edges.Files = []*Topic_File_Mats{} },
			func(n *Topic, e *Topic_File_Mats) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TopicQuery) loadCourse(ctx context.Context, query *CourseQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Course)) error {
...
func (tq *TopicQuery) loadFiles(ctx context.Context, query *TopicFileMatsQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Topic_File_Mats)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(topic_file_mats.FieldTopicID)
	}
	query.Where(predicate.Topic_File_Mats(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(topic.FilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TopicQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withCourse != nil {
			_spec.Node.AddColumnOnce(topic.FieldCourseID)
		}
	}
